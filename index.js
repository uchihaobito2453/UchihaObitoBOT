		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD デ❍多ቾ乙ቾ₦ BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] デ❍多ቾ乙ቾ₦? Este é um recurso especial para o デ❍多ቾ乙ቾ₦ ❌',
					ownerB: '[❗] デ❍多ቾ乙ቾ₦? Este é um recurso especial para o デ❍多ቾ乙ቾ₦ ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de デ❍多ቾ乙ቾ₦. Registre-se para fazer amizade com o bot デ❍多ቾ乙ቾ₦ por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar デ❍多ቾ乙ቾ₦|17\n\n──「 デ❍多ቾ乙ቾ₦ BOT 」──`,
			...
			const ownerNumber = ["558688275801@s.whatsapp.net","19566087481@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"558688275801@s.whatsapp.net","19566087481@s.whatsapp.net","15719012332@s.whatsapp.net","5521966305536@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["558688275801@s.whatsapp.net","19566087481@s.whatsapp.net","15719012332@s.whatsapp.net","5521966305536@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["558688275801@s.whatsapp.net","19566087481@s.whatsapp.net","15719012332@s.whatsapp.net","5521966305536@s.whatsapp.net"]//ubah nomor lo
			const premium = ["558688275801@s.whatsapp.net","19566087481@s.whatsapp.net","12792033668@s.whatsapp.net","558981427448@s.whatsapp.net","557182198257@s.whatsapp.net","15719012332@s.whatsapp.net","5521966305536@s.whatsapp.net","558694700993@s.whatsapp.net"] 
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("vsfd")){
			client.updatePresence(from, Presence.composing)
			reply("vai vc seu corno do karai")
	}
		...
		if (messagesC.includes("bot")){
			...
			switch(command) {
				...
				case 'menu':
                    ...
                    putagg = await getBuffer(`https://i.ibb.co/TwChfBK/IMG-20210508-WA0296.jpg`)
                   ...
                   case 'supergay':
					...
					if (!isOwner) return reply('Comando so para o meu dono, qm e vc?')
                     if (!isGroup) return reply(mess.only.group)
                      membr = []
                      const sorteio12 = groupMembers
                      const sorteio11 = groupMembers
                      const sortelho12 = sorteio12[Math.floor(Math.random() * sorteio12.length)]
                      const sortelho11 = sorteio11[Math.floor(Math.random() * sorteio11.length)]
                      reply(`Sortiados são vcs\n\n@${sortelho11.jid.split('@')[0]} @${sortelho12.jid.split('@')[0]}`)
                      membr.push(sorteio11.jid)
                      membr.push(sorteio12.jid)
                      mentions(teks, membr, true)
                       break
                    case 'gostosa':
                     if (!isGroup) return reply(mess.only.group)
...
if (isLimit(sender)) return reply(limitend(pushname2))
	if (args.length < 1) return reply("Coloque um texto na frente humm")
					...
					case 'hidetag2':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply('Quem é Você?')
				...
				break
				case 'shit':
				 data = fs.readFileSync('./shit.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                hasil = await getBuffer(randKey.result)
                sendImage(hasil, mek, '*shit*')
				break
					case 'wallpaper':
				 data = fs.readFileSync('./wallpaper.js');
                 jsonData = JSON.parse(data);
                 randIndex = Math.floor(Math.random() * jsonData.length);
                 randKey = jsonData[randIndex];
                hasil = await getBuffer(randKey.result)
                sendImage(hasil, mek, '*Ae man gostou?*')
				break
case 'tfs':
						if (!isGroup) return reply(mess.only.group)
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS デ❍多ቾ乙ቾ₦ BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @Causs domina vida
          ...
          reply('Espere..')
				...
				if (!isOwner) return reply(mess.only.ownerB)
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o デ❍多ቾ乙ቾ₦ ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT デ❍多ቾ乙ቾ₦ ✨* 」\n*│+ wa.me/5547992091566*╰──────*「 *デ❍多ቾ乙ቾ₦* 」*────*\n\n*_SE QUER SER ADMIN DO BOT デ❍多ቾ乙ቾ₦*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/554792091566/*╰──────*「 * デ❍多ቾ乙ቾ₦* 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO デ❍多ቾ乙ቾ₦ BOT_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do デ❍多ቾ乙ቾ₦:*\n\n http://youtube.com/channel/UCIrqmPMH60G2afNmSrT_B0A'})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
